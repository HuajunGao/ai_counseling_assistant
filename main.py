...
def main():
    ...
    def index():
        ...
        with ui.header().classes("bg-white text-gray-800 border-b p-4 flex items-center justify-between"):
            ...
            with ui.row().classes("items-center gap-4"):
                ...
                # Controls
                start_btn = ui.button(
                    "Start Session", on_click=lambda: start_session(device_select.value), icon="mic"
                ).props("color=green")
                stop_btn = (
                    ui.button("Stop", on_click=lambda: stop_session(), icon="stop")
                    .props("color=red")
                    .bind_visibility_from(globals(), "is_recording")
                )
        ...
        with ui.row().classes("w-full h-[calc(100vh-100px)] p-4 gap-4"):
            ...
            with ui.card().classes("w-1/3 h-full flex flex-col bg-slate-50"):
                ...
                ui.button(
                    "Refresh Suggestions",
                    on_click=lambda: generate_suggestion_task(suggestion_container),
                    icon="refresh",
                ).classes("w-full mt-2")
    ...
    def test_page():
        ...
        test_devices = {d["id"]: f"{d['id']}: {d['name'][:40]} ({d['api'][:10]})" for d in all_devices}
        ...
        with ui.card().classes("mx-auto mt-8 p-6 max-w-2xl"):
            ...
            with ui.row().classes("w-full items-end gap-4 mb-4"):
                ...
                loopback_toggle = ui.switch("Loopback Mode", value=True).tooltip(
                    "Enable for Speaker devices (capture system audio)"
                )
        ...
        with ui.column().classes("w-full min-h-screen p-4"):
            ...
            with ui.card().classes("w-full p-4 mb-4"):
                with ui.row().classes("w-full gap-4 items-end"):
                    mic_select = ui.select(mic_options, label="麦克风 (我)", value=0 if mic_options else None).classes(
                        "flex-grow"
                    )
                    speaker_select = ui.select(
                        speaker_options, label="扬声器 (对方)", value=0 if speaker_options else None
                    ).classes("flex-grow")
